...
[사례 모음 열기](cases/) → 과목 선택 → 사례 목록 → 이야기 순서로 읽습니다. 현재 **IT비즈니스와 윤리 11편(실제 사례 4편·가상 사례 7편)**을 제공합니다. 각 사례의 교훈은 교재 원리와 사례에 적용한 설명을 하나의 박스에 담으며, 원리의 교재 단원·페이지를 함께 확인할 수 있습니다.
...
`cases/`는 과목 선택, `cases/05/`는 사례 목록, `cases/05/BIZ-11/`과 `BIZ-01/`–`BIZ-10/`은 각 본문입니다. 로컬 서버 4186 포트에서 `npm run test:cases`로 PC·모바일 이동과 본문을 확인합니다. 배포본 검사는 `CASES_BASE_URL`을 공개 허브 주소로 지정합니다.

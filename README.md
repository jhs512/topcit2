...
단일 플래그는 `cases/features.mjs`의 **`enableCaseSpeech = false`**입니다. 사용자가 활성화를 요청하면 이 값만 `true`로 바꾸고 `npm run build:cases` 실행 후 생성 결과와 함께 배포합니다. 끌 때는 `false`로 되돌린 뒤 같은 절차를 따릅니다. 꺼져 있으면 사례에 음성 CSS/JS를 넣지 않아 버튼·레이어·강조·음성 초기화가 모두 없습니다. 학생 화면에는 설정 스위치를 제공하지 않습니다.

켜면 문단 옆 ▶ 버튼으로 해당 블록만 읽습니다. 재생·일시정지 중에는 화면 정중앙의 작은 고정 레이어에서 일시정지·이어읽기·정지·속도를 조절합니다. 배경을 막거나 포커스를 가두지 않습니다. 닫기·정지·레이어 안에서 Esc는 재생을 멈추고 누른 블록 버튼으로 포커스를 돌려줍니다. 완료 시 레이어가 닫힙니다. 이어읽기는 멈춘 문장부터, 속도 변경은 다음 문장부터 적용됩니다. 한국어 음성이 필요하며 기기별 재생 성능 차이는 그대로 있습니다. 별도 외부 유료 서비스는 사용하지 않습니다.

`npm run test:speech`는 분할·상태·취소·오류 검사를 실행합니다. 로컬 서버 4186에서 `npm run test:speech-feature`는 OFF→ON(모의 음성, PC·모바일 고정 위치와 조작)→OFF를 검사하고 마지막에 플래그와 생성 결과를 꺼진 상태로 복원합니다. 공개 OFF 검사는 `SITE_BASE`를 공개 주소로 지정해 `npm run test:speech-disabled`를 실행합니다. 실제 오디오 청취 검증은 모의/API 이벤트 검사와 별개입니다.

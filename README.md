...
발화가 시작되면 현재 읽는 문장 범위를 강조합니다. 긴 문장을 나눈 경우 현재 발화 조각을 표시합니다. 일시정지 중에는 위치를 유지하고 정지·종료·TTS 끄기·내용 변경·페이지 이동 시 해제합니다. 원본 DOM과 링크 이벤트를 유지하는 CSS Highlight API를 사용하며, 미지원 브라우저는 Range 좌표 표시로 대체합니다. 자동 스크롤은 하지 않습니다.

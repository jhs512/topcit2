...
모든 페이지의 공통 상단에 **TTS 꺼짐 / TTS 켜짐** 버튼이 있습니다. 첫 방문은 꺼짐이며, 켜면 본문의 읽기 버튼이 나타납니다. 실제 낭독은 개별 읽기 버튼을 눌러야 시작합니다. 상단에서 끄면 재생을 취소하고 중앙 패널·읽기 버튼·강조 표시를 정리합니다.
...
`npm run test:speech`는 분할·상태 검사를 실행합니다. 로컬 4186 서버에서 `npm run test:speech-feature`는 새 브라우저 프로필의 기본 OFF, ON→OFF→재ON, 저장·다중 탭 동기화, 비동기 로드 중 OFF, 모의 음성을 이용한 전체 페이지·동적 문제·숨김·중앙 패널을 검사합니다. 저장소 파일이나 기본값을 바꾸지 않습니다. `SITE_BASE`를 지정하면 공개 사이트에서도 같은 검사를 실행합니다. 공개 OFF 검사는 `SITE_BASE`에 공개 주소를 지정해 `npm run test:speech-disabled`를 실행합니다. 실제 오디오 청취는 모의/API 이벤트 검사와 별개입니다.

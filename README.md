...
| 소프트웨어 개발 | 1,095 | 36.5% |
| 데이터 관리 | 795 | 26.5% |
| 시스템아키텍처 및 정보보안 | 705 | 23.5% |
| IT비즈니스 | 405 | 13.5% |

[2026년 공식 시험안내 PDF](https://swuniv.korea.ac.kr/bbs/swuniv/979/177637/download.do)의 전체 배점을 연습량에 반영했습니다. **실제 객관식은 네 영역 각각 15문항(25%)**이므로 위 비율을 객관식 출제 문항 비율로 해석하지 않습니다. 8개 과목 내부 배분은 자체 기준이며 공식 세부 비율이 아닙니다.

추가한 2,400문항은 개념·상황 211개, Python 연산 순서 1,052개, SQL 조건·집계 732개, 페이지 교체 120개, CPU 스케줄링 80개, 접근통제 규칙 205개입니다. 연산·조건이 다른 연습 변형을 포함하므로 3,000개의 서로 다른 개념을 뜻하지 않습니다. 세부 주제의 비중은 실제 시험 비중을 재현하지 않습니다. [확충 계획과 검증 기록](docs/2026-09-16-180036-weighted-question-expansion-plan.md).
...
GitHub Pages 정적 디렉터리 경로를 사용하므로 끝의 슬래시가 없으면 슬래시가 붙은 주소로 이동합니다. 01·02를 포함한 여덟 과목에서 문제를 풉니다. 소프트웨어는 01, 데이터는 02에 대응합니다. 나머지 문항도 출제기준 번호로 분류합니다. 3.1.1.3 및 3.6–3.7은 04, 나머지 기술영역은 03입니다. 비즈니스영역의 4.1–4.2는 05-01, 4.3은 05-02, 4.4.2.2는 06-02, 나머지 4.4–4.5는 06-01입니다. 출제기준은 코드와 공식 기준 문장으로 표시합니다. 문항 ID와 개정 번호는 유지하여 학습기록을 이어갑니다.

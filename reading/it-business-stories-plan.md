...
아래 교훈 한 문장을 각 산문의 ‘이 글에서 배우는 교훈’에 그대로 쓴다. 실제 사례의 교훈은 사실에서 도출한 수업용 해석이며 원문 인용이 아니다.
...
각 편은 제목·사례 유형·교재 개념을 메타데이터로 보존하고, **이 글에서 배우는 교훈 → 들어가기 전에(선택) → 본문 → 결론** 순서로 구성한다. 결론은 공통 박스로 표시하고 출처는 별도 참고 영역에 둔다. [사례 작성 형식](CASE-AUTHORING.md)에 따라 Markdown을 작성하며 스키마 검증과 공통 렌더러로 생성한다. 교훈과 별도의 유사한 핵심 문장을 반복하지 않는다. ID와 교훈은 기획과 결과에서 일치시킨다.

...
import { enableCaseSpeech } from './features.mjs';
...
if (article && enableCaseSpeech) {
  ...
  panel.insertAdjacentHTML('afterbegin', '<button type="button" class="speech-close" aria-label="읽어주기 닫기 및 정지">닫기 ×</button>');
  document.body.append(panel);
  const notice = document.createElement('p');
  notice.className = 'speech-notice'; notice.setAttribute('role', 'status');
  notice.textContent = '한국어 음성을 확인하고 있습니다.';
  article.querySelector('header').after(notice);
  const close = panel.querySelector('.speech-close');
  ...
  if (!('speechSynthesis' in window) || !('SpeechSynthesisUtterance' in window)) {
    ...
    notice.textContent = '이 브라우저는 읽어주기를 지원하지 않습니다. 다른 브라우저에서 열어 주세요.';
  } else {
    ...
    const dismiss = () => {
      ...
      controller.stop();
      if (restoreFocus) originButton?.focus({ preventScroll: true });
    };
    const controller = new StorySpeech(synth, window.SpeechSynthesisUtterance, [], ({ state, message }) => {
      ...
      notice.hidden = !panel.hidden;
      if (panel.hidden) notice.textContent = message;
      ...
      if (panel.hidden && restoreFocus) originButton?.focus({ preventScroll: true });
    ...
    const refreshVoices = () => {
      ...
      if (voice) {
        ...
        notice.textContent = `문단 옆 ▶ 버튼으로 읽습니다 · ${voice.name}`;
      }
    };
    voiceTimer = setTimeout(() => {
      if (controller.state === 'idle' && !koreanVoice(synth.getVoices())) notice.textContent = '한국어 음성을 찾지 못했습니다. 기기의 한국어 음성을 확인한 뒤 본문의 읽기 버튼을 눌러 주세요.';
    }, 5000);

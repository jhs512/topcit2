...
for(const n of ['01','02','03','04','05','06']){
 ...
 if(Number(n)<3)await page.getByText('문제 준비 중입니다.').waitFor();
  ...
  assert.equal(await page.locator('.question-context a').count(),1);
  ...
  assert.equal(await page.locator('.reading-explanation a').count(),await page.locator('.reading-explanation').count());

...
try {
  for (const width of [1280, 390, 320]) for (const theme of ['light', 'dark']) {
    ...
    const buttons = page.locator('#context-fixture .block-speech-button'); await buttons.first().waitFor();
    ...
    await page.locator('#context-fixture p').nth(1).locator('.block-speech-button').click(); await start(); assert.deepEqual(await lines(), ['없음', '부호 없는 제목 본문링크', '없음']);

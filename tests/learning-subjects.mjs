...
const questions=(await Promise.all(['business','systems-security'].map(id=>readFile(new URL(`../practice/data/${id}.json`,import.meta.url),'utf8').then(JSON.parse)))).flat();
...
try{
  for(const width of [1440,720,390,320]){
    ...
    await page.getByRole('button',{name:'설명모드',exact:true}).click();assert.equal(await page.locator('.question-card').count(),20);
    ...
    await page.goto(url('practice/05/?mode=all#question-'+seeds[0].id));await page.locator('.history-target').waitFor();assert.equal(await page.locator('.question-card').count(),180);

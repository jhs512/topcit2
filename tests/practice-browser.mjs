...
const base=process.env.PRACTICE_BASE||'http://127.0.0.1:4182';
const business=JSON.parse(await readFile(new URL('../practice/data/business.json',import.meta.url),'utf8'));
const systems=JSON.parse(await readFile(new URL('../practice/data/systems-security.json',import.meta.url),'utf8'));
...
try{
 for(const width of [1440,390]){
  ...
  await page.goto(`${base}/practice/index.html`);await page.locator('[data-area="business"]').waitFor();assert.equal(await page.locator('.area-card').count(),2);
  await page.locator('[data-area="business"]').click();
  ...
  await page.getByRole('button',{name:'전체모드',exact:true}).click();assert.equal(await page.locator('.reading-list article').count(),20);assert.equal(await page.locator('.reading-explanation').count(),0);await page.locator('#next-page').click();assert.match(await page.locator('.reading-intro p').innerText(),/21–40/);

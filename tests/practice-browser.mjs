...
try{
 for(const width of [1440,390]){
  ...
  await page.getByRole('button',{name:'설명모드',exact:true}).click();assert.equal(await page.locator('.reading-explanation').count(),business.length);await page.locator('#topic').selectOption({label:business[0].topic});assert.ok(await page.locator('.reading-list article').count()>0);assert.match(await page.locator('.progress-label').innerText(),/완료 1개/);

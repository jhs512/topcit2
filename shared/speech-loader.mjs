...
export function initializeTts(button) {
  ...
  async function setEnabled(next, persist = false) {
    ...
    if (cleanup) return;
    try {
      modulePromise ||= import('./speech.mjs?v=20260916-sentence-context').catch(error => { modulePromise = undefined; throw error; });
      const speech = await modulePromise;
      if (!enabled || token !== revision) return;
      style = document.createElement('link');
      style.rel = 'stylesheet'; style.href = new URL('./speech.css?v=20260916-sentence-context', import.meta.url).href;

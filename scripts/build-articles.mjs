...
try{
  ...
  for(const article of articles){
    ...
    const html=shell(`${article.title} · 심화수업`,'../',`<div class="reading-progress" aria-hidden="true"><i></i></div><main id="main"><header class="article-hero"><a class="back" href="../">← 심화수업 전체 글</a><p class="eyebrow">${article.id} · ${esc(subject.title)}</p><h1>${esc(article.title)}</h1><p class="deck">${esc(article.deck)}</p><p class="reading-meta">${esc(article.scene)} · 약 ${minutes}분 · 그림 ${diagrams.length}개 · 표 ${tableCount}개</p><p class="fiction-label">한끼픽 실무 이야기 · 가상 상황을 따라 배우는 글입니다.</p></header><div class="article-layout"><aside class="article-toc"><details open><summary>이야기 순서</summary><ol>${headings.map(h=>`<li><a href="#${h.id}">${h.text}</a></li>`).join('')}</ol></details></aside><article class="prose" data-tts-content>${body}</article></div><section class="other-stories"><h2>같은 서비스를 다른 관점에서 읽기</h2><div>${peers}</div></section></main><dialog class="diagram-dialog" aria-labelledby="diagram-title"><div class="dialog-toolbar"><h2 id="diagram-title">그림</h2><button type="button" data-close>닫기</button></div><p>그림이 크면 가로·세로로 넘겨 읽으세요. Esc 키로 닫을 수 있습니다.</p><div class="zoom-scroll" tabindex="0" role="region" aria-label="확대한 그림"><img alt=""></div><a class="original-diagram" target="_blank" rel="noopener">원본 SVG 열기 ↗</a></dialog>`);
  ...
  await writeFile(resolve(root,'article/index.html'),shell('심화수업 · 처음부터 따라가는 실무 이야기','',`<main id="main"><header class="series-hero"><p class="eyebrow">심화수업 / 한끼픽 실무 이야기</p><h1>처음에는<br>주문 버튼 하나였습니다.</h1><p class="deck">한 가게의 작은 서비스가 자라면서 생긴 문제들.<br>무엇을 보고, 왜 바꿨고, 그 뒤에 무슨 일이 생겼는지 따라갑니다.</p><p>IT를 조금 아는 독자를 위한 긴 글 8편입니다. 각 글은 처음부터 독립적으로 읽을 수 있습니다. UML·구성도·표를 따라가며 실무의 판단을 살펴보세요.</p><a class="start-story" href="03/">서버 한 대로 시작하는 이야기부터 →</a></header><section class="article-grid" aria-label="8개 과목의 실무 이야기">${cards}</section><p class="series-note">사람·서비스·관측 수치는 설명용 가상 설정입니다. 실제 제품의 동작은 글 속 공식 문서 링크에서 확인할 수 있습니다. 교재 순서나 공식 출제 중요도를 뜻하지 않습니다.</p></main>`));

...
for (const s of subjects) {
  ...
  const body = `<a href="../">← 전체 핵심노트</a><header class="intro"><div class="eyebrow">핵심노트 · 주제 ${s.id}</div><h1>${escape(s.title)}</h1><p>${escape(s.intro)}</p><p class="editor-note">${note}</p></header><nav class="contents" id="contents" aria-label="핵심 개념 목차"><h2>핵심 개념 ${s.items.length}가지</h2><ol>${s.items.map(([title], i) => `<li><a href="#concept-${i + 1}">${escape(title)}</a></li>`).join('')}</ol></nav><div class="concepts">${article}</div><aside class="references"><h2>더 읽어 보기</h2><p><a href="../../cases/${s.id}/">이 과목의 사례 20편으로 적용해 보기 →</a></p><p>아래 자료는 관련 원칙을 더 살펴보는 참고 자료입니다. 이 글의 개념 선정이나 순위를 제시한 자료는 아닙니다.</p><ul>${references[s.id].map(([label, url]) => `<li><a href="${escape(url)}">${escape(label)}</a></li>`).join('')}</ul></aside><nav class="subject-links" aria-label="다른 주제의 핵심노트">${subjects.filter(other => other.id !== s.id).map(other => `<a href="../${other.id}/">${escape(other.title)}</a>`).join('')}</nav>`;

...
function renderStory(body) {
  const parts = body.split(/<!-- prerequisites:(start|end) -->/);
  let open = false;
  const html = parts.map(part => {
    if (part === 'start') {
      if (open) throw new Error('Nested prerequisite region');
      open = true;
      return '<section class="prerequisites" aria-label="이야기를 읽기 전에">';
    }
    if (part === 'end') {
      if (!open) throw new Error('Missing prerequisite start');
      open = false;
      return '</section>';
    }
    return marked.parse(part);
  }).join('');
  if (open) throw new Error('Missing prerequisite end');
  return html.replace(/<(h[1-6]|p|li)>/g, '<$1 class="tts-readable">');
}
...
await save('cases/05/index.html', page(subject, 2, `<nav class="breadcrumbs" aria-label="현재 위치"><a href="../../">홈</a><a href="../">사례 모음</a><span>${subject}</span></nav><section class="intro"><div class="eyebrow">교재 05 · ${counts}</div><h1 class="tts-readable">${subject}</h1><p class="tts-readable">프로그램이 잘 돌아가는데, 회사는 왜 힘들까?</p><p class="tts-readable">실제 사례는 공개 기록을 바탕으로, 가상 사례는 개념을 설명하기 위한 설정으로 작성했습니다.</p></section><div class="case-list">${cases.map(c => `<a class="area-card" href="${c.id}/"><div class="card-top">${c.id} · ${esc(c.type)}</div><h2 class="tts-readable">${esc(c.title)}</h2><p class="concept tts-readable">${esc(c.concept)}</p><p class="lesson tts-readable">${esc(c.lesson)}</p><span class="card-action">이야기 읽기 →</span></a>`).join('')}</div>`));
for (const [i, c] of cases.entries()) {
  await save(`cases/05/${c.id}/index.html`, page(c.title, 3, `<nav class="breadcrumbs" aria-label="현재 위치"><a href="../../../">홈</a><a href="../../">사례 모음</a><a href="../">${subject}</a><span>${c.id}</span></nav><article class="story"><header><div class="eyebrow">${subject} · ${c.id}</div><h1 class="tts-readable">${esc(c.title)}</h1></header>${renderStory(c.body)}</article><nav class="story-navigation" aria-label="사례 이동">${i > 0 ? `<a href="../${cases[i - 1].id}/">← 이전 사례<span>${esc(cases[i - 1].title)}</span></a>` : '<span></span>'}<a href="../">사례 목록</a>${i < cases.length - 1 ? `<a href="../${cases[i + 1].id}/">다음 사례 →<span>${esc(cases[i + 1].title)}</span></a>` : '<span></span>'}</nav>`));
}

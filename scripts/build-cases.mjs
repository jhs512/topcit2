...
for (const [i, c] of cases.entries()) {
  await save(`cases/05/${c.id}/index.html`, page(c.title, 3, `<nav class="breadcrumbs" aria-label="현재 위치"><a href="../../../">홈</a><a href="../../">사례 모음</a><a href="../">${subject}</a><span>${c.id}</span></nav><article class="story"><header><div class="eyebrow">${subject} · ${c.id}</div><h1>${esc(c.title)}</h1></header>${renderStory(c.body)}</article><nav class="story-navigation" aria-label="사례 이동">${i > 0 ? `<a href="../${cases[i - 1].id}/">← 이전 사례<span>${esc(cases[i - 1].title)}</span></a>` : '<span></span>'}<a href="../">사례 목록</a>${i < cases.length - 1 ? `<a href="../${cases[i + 1].id}/">다음 사례 →<span>${esc(cases[i + 1].title)}</span></a>` : '<span></span>'}</nav>`));
}
